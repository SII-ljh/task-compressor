...
def run_bottleneck(
    ...
    print("\n" + "=" * 76)
    print("  EXPERIMENT 3: Information Bottleneck (Random vs Compressed)")
    print("=" * 76)
    ...
    with torch.no_grad():
        ...
        labels = dec_out["labels"][:, 1:].contiguous()
...
def main():
    ...
    if data_path is None:
        for p in ["data/qa_dev.json", "data/qa_tiny.json", "data/qa_train.json"]:
            if os.path.exists(p):
                data_path = p
                break
        if data_path is None:
    ...
    print(f"\nPreparing single QA batch from {data_path}...")
