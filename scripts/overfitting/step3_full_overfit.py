...
Train Task Compressor on 5% ablation data with 90/10 train/val split.
Includes optional distillation. Tracks train loss + val loss + val PPL.

Usage:
  # Default: 20K steps on ablation subset
  python scripts/overfitting/step3_full_overfit.py

  # Custom data and steps
  python scripts/overfitting/step3_full_overfit.py \
      --data_path data/qa_dev.json --steps 5000

  # With distillation
  python scripts/overfitting/step3_full_overfit.py --distill_alpha 0.5

  # Resume from checkpoint
  python scripts/overfitting/step3_full_overfit.py --checkpoint outputs/overfit_step3/best.pt
"""
...
@torch.no_grad()
def evaluate_qa(model, loader, device, use_bf16=False, distill_alpha=0.0):
    """Compute average QA loss and perplexity on a loader."""
    model.eval()
    total_loss = 0.0
    total_qa_loss = 0.0
    total_samples = 0
    ...
    for batch in loader:
        ...
        total_qa_loss += outputs["qa_loss"].float().item() * bs
        total_samples += bs

    avg_loss = total_loss / max(total_samples, 1)
    avg_qa_loss = total_qa_loss / max(total_samples, 1)
    ppl = math.exp(min(avg_qa_loss, 20))
    model.train()
    return {"loss": avg_loss, "qa_loss": avg_qa_loss, "perplexity": ppl}


# ── Training loop ─────────────────────────────────────────────────────

def train(model, train_loader, val_loader, device, args, use_bf16=False):
    """QA training loop with periodic val evaluation."""
    ...
    csv_writer.writerow([
        "step", "train_loss", "val_loss", "val_qa_loss", "val_ppl",
        "lr", "elapsed_s"
    ])
    ...
    if val_loader is not None:
        ...
        logger.info(f"[init] val_loss={metrics['qa_loss']:.4f}  val_ppl={metrics['perplexity']:.1f}")
        csv_writer.writerow([0, "", f"{metrics['qa_loss']:.4f}",
                             f"{metrics['qa_loss']:.4f}", f"{metrics['perplexity']:.1f}",
                             f"{args.lr:.2e}", "0"])
        csv_file.flush()
        ...
        for batch in train_loader:
            ...
            if micro_steps % args.grad_accum == 0:
                ...
                if val_loader is not None and step % args.eval_every == 0:
                    ...
                    logger.info(
                        f"[EVAL] step {step}  val_loss={metrics['qa_loss']:.4f}  "
    ...
    if val_loader is not None:
        ...
        if metrics["perplexity"] < best_ppl:
            ...
            _save_trainable(model, os.path.join(args.output_dir, "best.pt"))
        logger.info(
            f"[FINAL] val_loss={metrics['qa_loss']:.4f}  "
...
def main():
    ...
    parser.add_argument("--distill_alpha", type=float, default=0.0,
                        help="Distillation weight (0 = no distillation)")
    ...
    # Summary
    print("\n" + "=" * 60)
    print("  STEP 3: FULL OVERFIT TEST WITH TRAIN/VAL SPLIT")
    print("=" * 60)
